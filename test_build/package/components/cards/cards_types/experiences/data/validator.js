...
var WorkValidator = function WorkValidator(formatMessage) {
    return Yup.object().shape({
        work: Yup.array()
            .required(formatMessage(_validator_translations.workTranslations.atLeastOne))
            .min(1, formatMessage(_validator_translations.workTranslations.atLeastOne))
            .of(
                Yup.object()
                    ...
                    .shape({
                        position: Yup.string().required(
                            formatMessage(_validation_translations.validationTranslations.required)
                        ),
                        name: Yup.string().min(
                            5,
                            formatMessage(_validation_translations.validationTranslations.min, {
                                min: 5
                            })
                        ),
                        summary: Yup.string()
                        ...
                        startDate: Yup.object()
                            ...
                            .test(
                                'is-not-in-future',
                                formatMessage(_validator_translations.workTranslations.noFutureDate),
                                function (value) {
                        ...
                        endDate: Yup.object().when('stillEmployed', {
                            ...
                            otherwise: Yup.object().when('startDate', function (start) {
                                return Yup.object()
                                    ...
                                    .test(
                                        'isafter',
                                        formatMessage(_validation_translations.validationTranslations.isAfter),
                                        function (value) {
                                            if (
                                                !start ||
                                            ...
                                            return (0, _moment.default)(value).isSameOrAfter(start);
                                        }
                                    );
                            })
                        })
